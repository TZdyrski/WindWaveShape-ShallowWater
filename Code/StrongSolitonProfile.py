...
def FullProfile(press_type, phase, P=1, psi_P=np.pi/2, depth=np.inf,
    ...
    if press_type == "jeffreys":
        sechTerm = 1/np.cosh(np.sqrt(3*c1/8)*(phase+0*t*(c1*eps/4)))
        tanhTerm = np.tanh(np.sqrt(3*c1/8)*(phase+0*t*(c1*eps/4)))

        firstOrderTerm = c1*sechTerm**2
        secondOrderTerm = -P*c1**3*3/128/np.pi**2*t*sechTerm**4 \
                *(9-11*sechTerm**2-6*np.sqrt(3*c1/8)*t*(1+c1*eps/2)*tanhTerm)

## This gives the same result, but it isn't as simplified (to check,
## ensure t is non-zero)
#        sech = 1/np.cosh(np.sqrt(3*c1/8)*(phase+0*t*(c1*eps/4)))
#        tanh = np.tanh(np.sqrt(3*c1/8)*(phase+0*t*(c1*eps/4)))
#
#        A = c1*sech**2
...
#
#        firstOrderTermB = A
#        secondOrderTermB = -P/64/np.pi**2*t \
#                *(
#                        5*diffA**2
#                        +4*A*diffDiffA
#                        +3*t*(1+c1*eps/2)*(
...
#                 )
        ...
        sechTerm = 1/np.cosh(np.sqrt(3/8)*(phase+0*t*(eps/4)))
        tanhTerm = np.tanh(np.sqrt(3/8)*(phase+0*t*(eps/4)))
        sechTermPsiP = 1/np.cosh(np.sqrt(3/8)*(phase+0*t*(eps/4)+psi_P))
        tanhTermPsiP = np.tanh(np.sqrt(3/8)*(phase+0*t*(eps/4)+psi_P))

        firstOrderTerm = sechTerm**2
        secondOrderTerm = -P*c1**2/64/np.pi**2*np.sqrt(3*c1/8)*t \
                *(-18*sechTermPsiP**4*tanhTermPsiP
                  +8*sechTermPsiP**2*sechTerm**2*tanhTerm
                  +10*sechTermPsiP**2*sechTerm**2*tanhTermPsiP
                  +3*np.sqrt(3*c1/8)*t*(1+c1*eps/2)*(
                      4*sechTermPsiP**4*tanhTermPsiP**2
                      +2*sechTermPsiP**4*(2-3*sechTermPsiP**2)
                      -4*sechTermPsiP**2*tanhTermPsiP*sechTerm**2*tanhTerm
                      -2*sechTerm**2*sechTermPsiP**2*(
                          2-3*sechTermPsiP**2)
                      )
                  )
## This gives the same result, but it isn't as simplified (to check,
## ensure t is non-zero)
#        sech = 1/np.cosh(np.sqrt(3/8)*(phase+0*t*(eps/4)))
#        sechpsiP = 1/np.cosh(np.sqrt(3/8)*(phase+0*t*(eps/4)+psi_P))
#        tanh = np.tanh(np.sqrt(3/8)*(phase+0*t*(eps/4)))
#        tanhpsiP = np.tanh(np.sqrt(3/8)*(phase+0*t*(eps/4)+psi_P))
#
#        A = c1*sech**2
#        ApsiP = c1*sechpsiP**2
#        diffA = -2*tanh*sech**2*c1*np.sqrt(3*c1/8)
...
if(plot_profile):
    ...
    JPress = np.transpose(np.array(
        [FullProfile('jeffreys', xs, P=P, depth=profileDepth,
            eps=eps, t=time) for time in profile_times]))
    ...
    GPress = np.transpose(np.array(
        [FullProfile('generalized', xs, P=P, depth=depth, eps=eps,
            psi_P = psi_P, t=time) for time in profile_times]))
    ...
    texplot.savefig(fig,'../Figures/Solitary-Profile')
