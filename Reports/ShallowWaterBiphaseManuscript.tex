...
for offshore, damping wind and \cref{eq:kdv_burgers} is the (forward)
KdV-Burgers equation.
However, for onshore wine, the viscosity is negative and causes wave
...
of $\order{T_0/\epsilon}$ and we will therefore ignore the ill-posed
nature.
\end{LineLabel}
...
The simulation runs from $t_1 = 0$ to $t_1 = T_b \approx \num{19.29}$,
with $T_b$ the time at which the strongest onshore forcing $P=0.25$
causes the wave to experience convective breaking with $H/h =
\max{\eta}/h = 0.6879$~\citep{brun2018convective}.
\end{LineLabel}
...
\num{7.86e-3}$ and increases to \num{8.83e-3} for $P=-0.25$ and
decreases to \num{6.28e-3} for $P = 0.25$.
\end{LineLabel}
...
\begin{figure}
  ...
  \caption{
    ...
    $9.7$ and $19$, as indicated in the legend.
...
water level at $t\epsilon \sqrt{gh} k_E=19$ (green line).
...
\begin{figure}
  ...
  \caption{
    ...
    $t'_1 = t \epsilon \sqrt{gh} k_E = 0$, $9.7$ and $19$, as indicated
    in the legend.
    Only a subset of the full spatial domain is shown.
...
$t \epsilon \sqrt{g h} k_E = 19$, corresponding to $19/\epsilon = 190$
