...
\begin{equation*}
  ...
  \end{aligned}
\end{equation*}
We have defined the parameters $\epsilon \coloneqq H_0/(2h)$ and $\mu_E
\coloneqq (kh)^2$ and chosen $\order{P k_E/(\rho_w g)} =
\order{\epsilon}$.
...
Here, we modify the Bousinesq derivation provided by
...
\begin{gather}
  ...
  \epsilon P \partial_x \eta + \eta + \partial_t \varphi -
...
viscosity'' (which is expected, since the wind can cause either growth
or decay).
...
\subsection{Numerics}
To solve \cref{eq:kdvb_nondim}, we will use numerical methods.
...
This is to be expected, as \cref{sec:press_mag} suggests the
...
\subref{fig:statistics_solitary:c} asymmetry are defined as is defined
as
\begin{equation}
...
solitary wave profile \cref{eq:initial_cond_nondim}.
...
\Citet{donelan2006wave} provides a parameterization for data from
\citet{donelan1999wind} that yields $S_{\lambda/2} = 0.17$.
...
$z=\SI{10}{\meter}$, $U_{10} = \SI{29}{\meter\per\second}$.
...
\footnote{
  ...
  Using the standard definition of asymmetry \cref{eq:shape_stats_def},
  $\As \propto -\sin{\beta}$.
}
