...
bound wave and dispersive, decaying tail.
...
wind and shallow water waves have not yet been combined into a simple
equation such as the KdV-Burgers equation.
\end{LineLabel}
...
The solitary waves of the unforced ($P=0$) KdV equation exist due to a
balance of dispersion $\partial_x^3 \eta_0$ with focusing nonlinearity
$\eta_0 \partial_x \eta_0$ and have the
...
The pressure term $P \partial_x^2 \eta_0$ in \cref{eq:kdv_burgers}
...
We quantify the wave's shape using its energy $E$, skewness $\Sk$ and
asymmetry $\As$,
\begin{equation}
...
the unforced KdV soliton solution which results in a trailing, bound,
...
The resulting the pressure forcing term $P \partial_x^2 \eta$ preserves
the initial symmetry and induces a symmetric bound wave after a short
time $\Delta t'_1 \ll 1$.
This can be seen by considering the non-dimensional KdV-Burgers equation
\cref{eq:kdv_burgers} in the unforced solitary wave's frame
...
\begin{LineLabel}{R1-12}
Unlike dispersive tails generated from initial condition which fail to
satisfy the KdV equation, our wave is continually forced by the surface
pressure resulting in a growing, bound tail, as is apparent in
\cref{fig:snapshots_solitary_tail} (\eg $\tilde{x}/h =
\numrange{-20}{3}$ for $P'=0.25$ and $t'_1=10$) with the rear side
($x<0$) of the bound wave grows/widens with time.
\end{LineLabel}
...
profile, our tail is continually forced and lengthened by the wind
forcing.
\end{LineLabel}
...
Second, it quickly generates a bound wave and asymmetric, dispersive
tail behind the wave (\cref{fig:snapshots_solitary_tail}), producing a
...
g) \ll 1$.
This was the first analysis to produce a KdV-Burgers equation governing
the wave profile $\eta$ in shallow water, which we then solved
numerically with a symmetric, solitary wave initial condition.
\end{LineLabel}
