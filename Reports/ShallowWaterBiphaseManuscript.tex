...
Both the onshore wind (\cref{fig:snapshots_solitary:a}) and offshore
wind (\cref{fig:snapshots_solitary:b}) begin from an identical,
...
While an onshore (offshore) wind increases (decreases) the wave's local
steepness everywhere, the windward side of the wave always becomes steeper
than the leeward side (up to \SI{8}{\percent} steeper for the time
period shown).
Finally, while the front face ($x>0$) of the wave shows only a moderate
change in slope, the rear face ($x<0$) of the wave shows a qualitative
difference between the two wind directions.
...
We calculate the wave height (\cref{fig:statistics_solitary:a}) as
\begin{equation}
  H(t) = \max(\eta) - \min(\eta) \,.
  \label{eq:height_def}
\end{equation}
...
asymmetry.
The skewness (\cref{fig:statistics_solitary:b}) and
asymmetry (\cref{fig:statistics_solitary:c}) are
\begin{equation}
...
domain size $\sqrt{L'}$, we normalize the skewness $\Sk$ by the
initial skewness $\Sk_0$.
