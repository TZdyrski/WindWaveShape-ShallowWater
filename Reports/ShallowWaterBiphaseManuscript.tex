...
larger $\mu_E > 6 \epsilon$, though we will not do so for brevity (\cf
\cref{sec:comparison}).
\end{LineLabel}
...
The only notable differences were that the asymmetry seems to saturate
at a constant value and the pressure term needed to be
ramped-up over a wavelength-dependent period of time to prevent the
skewness and asymmetry from displaying oscillatory behavior.
