...
shift~\citep{husain2019boundary,hao2020mechanistic}.
...
\begin{figure}
  ...
  \caption{
    The difference $\Delta \eta/h$ between the surface profile and
    reference solitary wave \cref{eq:profile_discrepancy} under
    \subref{fig:snapshots_solitary_tail:a} onshore and
    \subref{fig:snapshots_solitary_tail:b} offshore Jeffreys forcing
    versus reference wave-centered distance $\tilde{x}/h$.
...
To further examine the wind-induced wave asymmetry, we subtract off an
unforced, solitary reference wave from $\eta$.
We calculate the reference solitary wave's height $H'(t'_1)$ and peak
location $x'_{\text{ref}}(t'_1)$ using a least-absolute-deviation fit
to leverage the cancellation of the oscillatory remainder.
After boosting to a frame centered on the reference solitary wave
$\tilde{x}' \coloneqq (x'-x'_{\text{ref}})$, we subtract the reference
solitary wave of the appropriate height from the profile $\eta$,
\begin{equation}
...
\end{equation}
This difference $\Delta \eta(x)$ is shown in
\cref{fig:snapshots_solitary_tail}.
Notice that the tail begins near the front face of the wave and decays
with distance behind the wave and has extrema for negative $\tilde{x}'$
but with opposite signs for opposite wind directions.
...
KdV systems \citep[\eg{}][]{mei2005nonlinear}.
