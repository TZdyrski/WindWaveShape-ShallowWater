...
$N_t = \num{60}$ points, yielding a spacing $\Delta t_1 = T/(N_t-1)
\approx \num{0.051}$.
...
We use the Dedalus spectral solver which implements a generalized tau
method and zero-padding with a scaling factor of $3/2$ to prevent
aliasing of nonlinear terms.
A Chebyshev basis is chosen with, and time stepping is done with a 3rd-order
4-stage DIRK+ERK scheme.
The normalized root-mean-square difference between the unforced ($P=0$)
...
\begin{figure}
  ...
  \caption{
    ...
    \subref{fig:terms:c},\subref{fig:terms:d} show just the wind and
    change terms.
    Results are shown for $\epsilon=0.1$, $\mu_E = 0.6$, $\abs{P
...
When wind forcing ($P \partial_{x'}^2 \eta'_0 /2$) is included, the wave
shape experiences a time-dependent change ($\partial_{t_1'} \eta'_0$).
...
the wind and change terms, which would each be zero in
the unforced case.
