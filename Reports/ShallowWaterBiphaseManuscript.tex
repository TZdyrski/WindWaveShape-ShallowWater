...
whose evolution from an initially symmetric, solitary wave initial
condition is calculated numerically.
...
$\eta_0 \partial_x \eta_0$ and have the form
...
We use \cref{eq:initial_condition} for our initial condition with
$t_1=0$ and choose $H_0 = 2$ so the initial, dimensional amplitude $a_0$
is half the wave height (\cf{} \cref{sec:nondim}).
The introduction of a dissipative term $P \partial_x^2 \eta_0$ in
\cref{eq:kdv_burgers} disrupts the solitary wave's balance of dispersion
and nonlinearity, inducing shape changes and growth or decay.
...
parameter $\psi_P/k$.
However, the GM forcing is only applicable to periodic waves, and these
...
condition.
From the numerical results, we estimated the energy, skewness and
