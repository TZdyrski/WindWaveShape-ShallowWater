...
with a secondary multiple-scale approximation of the KdV-Burgers
...
However, the surface pressure does not cause these changes directly.
Indeed, if the surface profile $\eta$ is symmetric (as it is in our
initial condition), then the pressure forcing term $P \partial_x^2 \eta$
...
\begin{equation}
  ...
  \sech^2\pqty{\frac{x'}{2}}} =  P' \bqty{
  \frac{3}{2}\sech^4\pqty{\frac{x'}{2}}
  -\sech^2\pqty{\frac{x'}{2}}
  }\,.
\end{equation}
Thus, after a short time $\Delta t'_1$, the wave profile will be
\begin{equation}
  \eta'_0(x', \Delta t'_1) =
...
\end{equation}
Though we assumed that $P' = Pk_E/(\rho_w g \epsilon) = \order{1}$, the
numerical value $\abs{P'} = 0.25$ we used in \cref{sec:results} was
moderately small.
Therefore, we can consider the wave's evolution as two components with
temporal scale separation between them.
