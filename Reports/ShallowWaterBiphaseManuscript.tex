...
\keywords{surface gravity waves, wind--wave interactions, air/sea
interactions}
...
Waves in shallow water, where $kh \ll 1$ (with $h$ the water depth,
...
solver, so time stepping is done with coupled four-stage, third-order
Diagonally Implicit Runge--Kutta and Explicit Runge--Kutta schemes.
...
Adaptive time stepping is employed such that the
Courant--Friedrichs--Lewy number is $(\Delta t) \max(\eta_0)/(\Delta x)
= 1$.
...
\begin{figure}
  ...
  \caption{
    ...
    magnitude $\abs{P k_E/(\rho_w g \epsilon)}$ up to $0.25$, as
...
increase (decrease) while the asymmetry decreased (increased) with time.
