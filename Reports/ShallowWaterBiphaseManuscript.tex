...
\begin{figure}
  ...
  \caption{
    ...
    versus peak-centered distance $(x-x_{\text{peak}})/h$.
...
\begin{equation}
  ...
  \label{eq:height_def}
\end{equation}
After boosting to a frame centered on the peak location
$x'_{\text{peak}}$ with coordinate $\tilde{x}' \coloneqq
(x'-x'_{\text{peak}}$, we subtract the solitary wave of the appropriate
height from the profile $\eta$,
\begin{equation}
...
\begin{figure}
  ...
  \caption{
    ...
    Results are shown for $\epsilon=0.1$, $\mu_E = 0.6$ and with with
