...
leveraging the small parameters $\order{a/h} = \order{(kh)^2}$ yields
the Boussinesq equations for waves in small waves in shallow water.
...
density.
We also used the gauge freedom to absorb the Bernoulli `constant' $C(t)$
in the dynamic boundary condition into the definition of $\phi$.
We are seeking a solitary, progressive wave with boundary conditions
\begin{gather}
...
(`onshore') wind in the same direction as the wave and $P<0$ denoting
...
We will non-dimensionize with the known characteristic scales: the
...
Denoting non-dimension variables with an prime, we have
\begin{equation*}
  \begin{aligned}
  ...
  \end{aligned}
\end{equation*}
We have defined the non-dimension parameters $\epsilon \coloneqq a_0/h$
and $\mu_E \coloneqq (k_E h)^2$ and chosen $\order{P k_E/(\rho_w g)} =
\order{\epsilon}$.
These three small, non-dimension parameters define our system.
...
\approx \num{6.25e-6}$.
We found that linearly ramping $P'$ up from $0$ at $t'_1=0$ to its full
value at $t'_1 = 0.1$ (or $t = \sqrt{gh} k_E$, \ie approximately one
wave-crossing time) did not qualitatively modify the results, so we do
not utilize such a ramp-up here.
...
\begin{figure}
  ...
  \caption{
    ...
    k_E/(\rho_w g \epsilon)} = 0.25$ non-dimension slow times $t
...
$t \epsilon \sqrt{g h} k_E$ when plotted against the non-dimension
distance $x/h$.
...
\begin{figure}
  ...
  \caption{
    ...
    Jeffreys forcing are shown for non-dimension slow time $t \epsilon
    \sqrt{gh} k_E = \numrange{0}{3}$.
    The
...
the non-dimension slow time $t \epsilon \sqrt{g h} k_E$ for onshore to
offshore wind with $P k_E/(\rho_w g \epsilon)$ varying between $\pm
0.25$.
We plot all cases for initial steepness $\epsilon = 0.1$ up to slow time
$t'_1 = 3$, corresponding to $3/\epsilon = 30$ wave-crossing times,
$\sqrt{gh} k_E$.
