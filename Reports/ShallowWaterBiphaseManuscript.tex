...
energy $E$ \cref{eq:shape_stats_def}, which we can approximate with the
...
Note that exponential energy growth \cref{eq:gamma_vs_P_solitary}
...
\cref{fig:snapshots_solitary,fig:statistics_solitary}.
...
\epsilon) = 0.25$.
\Citet{donelan2006wave} parameterize $S_z$ for
...
non-dimensional KdV-Burgers equation \cref{eq:kdv_burgers} in the frame
of unforced solitary wave (\cref{fig:snapshots_solitary}) at the initial
time,
\begin{equation}
  \begin{split}
...
with temporal scale separation.
First, the pressure adiabatically modifies the wave
\cref{eq:pressure_change}, and then the wave quickly evolves from this
new profile according to the unforced KdV evolution given by the inverse
scattering transform (IST).
The IST shows that any localized, initial profile differing from
...
solitary wave, which is reflected in the growth/decay and
...
the wave \cref{fig:snapshots_solitary_tail}, which explains the greater
shape change of the rear face in \cref{fig:snapshots_solitary}.
Finally, the wind direction (\ie{} pressure forcing sign) changes the
phase of the oscillatory tail, causing the negative/positive asymmetry
for onshore/offshore wind forcing in \cref{fig:statistics_solitary}.
