...
Instead, the pressure forcing initially causes a (symmetric) bound wave
after a short time $\Delta t'_1 \ll 1$, which can be seen by considering
the non-dimensional KdV-Burgers equation \cref{eq:kdv_burgers} in the
...
This bound wave is apparent in \cref{fig:snapshots_solitary_tail} (\eg
$\tilde{x}/h = \numrange{-20}{3}$ for $P'=0.25$ and $t'_1=10$), and the
rear side ($x<0$) of the bound wave grows/widens with time.
\end{LineLabel}
...
profile, our tail is continually amplified and lengthened by the
