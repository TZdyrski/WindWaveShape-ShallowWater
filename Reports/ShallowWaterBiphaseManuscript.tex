...
Furthermore, we now define what $k_E$ means by choosing $\mu_E = 6
\epsilon$, which is simply the well-known fact that the height and width
of a solitary wave satisfy a fixed relationship.
...
\begin{equation}
  \pdv{\eta_0}{t_1} + U\pdv{\eta_0}{x} + \frac{3}{2}
    \eta_0 \pdv{\eta_0}{x} + \pdv[3]{\eta_0}{x} = 0 \,.
  \label{eq:kdv}
\end{equation}
...
\citealp{brun2018convective}) the solitary wave solution to
\cref{eq:kdv} is
\begin{equation}
...
\end{equation}
with $\nu_{\text{bi}} = \num{3e-3}$, which we solve with initial
condition
\begin{equation}
