...
\begin{align}
  \eta(x,t) &= \sum_{k=0}^{\infty} \epsilon^k
    \eta_{k}(x,t_0,t_1,\ldots) \,, \\
  \varphi(x,t) &= \sum_{k=0}^{\infty} \epsilon^k
    \varphi_{k}(x,t_0,t_1,\ldots) \,, \\
  p(x,t) &= \sum_{k=0}^{\infty} \epsilon^k p_{k}(x,t_0,t_1,\ldots)
    \,.
\end{align}
...
If we choose a nearshore value of $h=\SI{2.5}{\meter}$ and a nonlinearity
parameter of $\epsilon = 0.5$, then the simulation is run for a time
period of $t = \SI{6}{\second}$.
Using an inner shelf value of $h=\SI{10}{\meter}$ with $\epsilon = 0.1$
gives a period of $t = \SI{13}{\second}$.
Decreasing the nonlinearity parameter to $\epsilon = 0.01$ and an inner
shelf depth of $h=\SI{10}{\meter}$ give a simulation length of
\SI{6}{\minute} \SI{52}{\second}.
